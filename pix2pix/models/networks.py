...
def define_G(input_nc, output_nc, ngf, netG, norm='batch', use_dropout=False, init_type='normal', init_gain=0.02, gpu_ids=[]):
    ...
    if netG == 'resnet_9blocks':
        ...
        #net = UnetGeneratorSmall(input_nc, output_nc, 5, ngf, norm_layer=norm_layer, use_dropout=use_dropout)
...
class UnetGeneratorSmall(nn.Module):
    ...
    def __init__(self, input_nc, output_nc, num_downs, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False):
        ...
        It is a recursive process.
        """
        super(UnetGeneratorSmall, self).__init__()
        # construct unet structure
        unet_block = UnetSkipConnectionBlock(ngf * 4, ngf * 4, input_nc=None, submodule=None,
                                             norm_layer=norm_layer, innermost=True)  # add the innermost layer
        # for i in range(num_downs - 5):          # add intermediate layers with ngf * 8 filters
        #    unet_block = UnetSkipConnectionBlock(ngf * 4, ngf * 4, input_nc=None,
        #                                         submodule=unet_block, norm_layer=norm_layer, use_dropout=use_dropout)
        # gradually reduce the number of filters from ngf * 8 to ngf
        # unet_block = UnetSkipConnectionBlock(ngf * 2, ngf * 4, input_nc=None,
        #                                     submodule=None, norm_layer=norm_layer)
        unet_block = UnetSkipConnectionBlock(ngf * 1, ngf * 4, input_nc=None,
                                             submodule=unet_block, norm_layer=norm_layer)
