...
class GraphGenerator(LiDARDataset):
    ...
    def point_sampler_build_dgl_graph(self, cam_center, ray_dirs, E, K):
        ...
        dist_sample_to_map_points, nearest_map_points_ind = self.map_kd_tree.query(
            sample0_xyz[ind_sel_samples], distance_upper_bound=self.knn_radius)
        ...
        dist_sample_to_map_points, nearest_map_points_ind = self.map_kd_tree.query(
            sample, k=self.max_num_map_points, distance_upper_bound=self.knn_radius)
        ...
        unique_ind_sample_to_map = torch.arange(sample.shape[0]).unsqueeze(
            1).repeat(1, self.max_num_map_points)[mask_map_points]
    ...
    def render_lidar_depth(self, g):
        ...
        def func_reduce(nodes):
            ...
            acc_transmission = torch.cumprod(
                torch.cat([torch.ones(opacity.shape[0], 1), 1. - opacity + 1e-10], dim=1), -1)[:, :-1]
